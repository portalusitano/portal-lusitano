...
| `rls-prova-antes.sql`    | Vinte e uma operações feitas como `anon`. Antes da migração passam todas.                                                                                    |
...
# Depois: 21 «bloqueado», zero «AINDA PASSA».
...
- **21 «bloqueado»** — cada uma das operações que `anon` conseguia fazer.
  Qualquer linha `AINDA PASSA` é uma falha da migração.
- **O que continua a funcionar** — `app/mapa/page.tsx` lê `coudelarias` activas
  com a chave anónima (é a única leitura anónima de tabela em todo o projecto),
  a ficha do vendedor lê `seller_ratings`, e o `service_role` — por onde o resto
  do site todo lê — continua a passar à frente do RLS.
